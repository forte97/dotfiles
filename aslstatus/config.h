...
#endif
        { username, "^B8^^C15^%s@",     		 NULL,	ONCE,	END},
        { hostname, "%s ",      		 NULL,	ONCE,	END},
        { uptime,   "^B15^^C8^ Uptime: %s ",	 NULL,	1 _MIN,	END},
        { run_command, "^B8^^C15^  : %s ",	 "nmcli con show --active | awk 'NR==2{print $1}'",	2 _SEC,	END},
        { run_command, "^B15^^C8^ 墳: %s%% ",	 "pamixer --get-volume",		2 _SEC,		END},
        { cpu_perc, "^B8^^C15^ CPU: %s%% ",     	 NULL,	1 _SEC,	END},
        { ram_used, "^B15^^C8^ MEM: %sB ",        NULL,	5 _SEC, END},
        { datetime, "^B8^^C15^  %s ",          	 "%b %d  %a",	10 _SEC,	END},
